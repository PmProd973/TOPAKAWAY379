                ...
                obj.children.forEach(childObj => {
                  if (childObj.type === 'piece') {
                    ...
                    const childMaterial = new THREE.MeshStandardMaterial({
                      color: childObj.piece?.color || '#ebe7dd',
                    ...
                    // ----- DÉBUT MODIFICATION POUR DÉTECTION DES FILEURS -----
                    // Fonction améliorée de détection des fileurs
                   // BLOC AMÉLIORÉ pour détecter et positionner les fileurs
const isFillerType = 
  (childObj.piece && childObj.piece.type === 'filler') || 
  (childObj.piece && childObj.piece.description && 
   childObj.piece.description.toLowerCase().includes('fileur')) ||
  (childObj.id && childObj.id.includes('cladding_') && 
   childObj.piece && childObj.piece.description && 
   childObj.piece.description.toLowerCase().includes('fileur'));

if (isFillerType) {
  // Déterminer le type de fileur avec une logique robuste
  let isLeftFiller = false;
  let isRightFiller = false; 
  let isTopFiller = false;
  
  // Vérifier d'abord le type de position explicite (le plus fiable)
  if (childObj.piece && childObj.piece.position) {
    isLeftFiller = childObj.piece.position === 'left';
    isRightFiller = childObj.piece.position === 'right';
    isTopFiller = childObj.piece.position === 'top';
  } 
  // Sinon, détecter par l'ID et la description
  else {
    const id = childObj.id ? childObj.id.toLowerCase() : '';
    const description = childObj.piece && childObj.piece.description 
      ? childObj.piece.description.toLowerCase() 
      : '';
    
    isLeftFiller = id.includes('left') || id.includes('gauche') || 
                   description.includes('gauche') || description.includes('left');
    
    isRightFiller = id.includes('right') || id.includes('droit') || 
                    description.includes('droit') || description.includes('right');
    
    isTopFiller = id.includes('top') || id.includes('supérieur') || 
                  description.includes('supérieur') || description.includes('top');
  }
  
  console.log(`Fileur détecté: ${childObj.piece.description}`, {
    id: childObj.id,
    isLeftFiller, 
    isRightFiller, 
    isTopFiller
  ...
  if (isLeftFiller || isRightFiller) {
    // Fileur latéral - rotation de 90° autour de Z
    childMesh.rotation.z = Math.PI / 2;
    console.log(`Rotation Z (${Math.PI/2} rad) appliquée à: ${childObj.piece.description}`);
  } else if (isTopFiller) {
    // Fileur supérieur - rotation de 90° autour de X
    childMesh.rotation.x = Math.PI / 2;
    console.log(`Rotation X (${Math.PI/2} rad) appliquée à: ${childObj.piece.description}`);
  }
}

                    // ----- FIN MODIFICATION POUR DÉTECTION DES FILEURS -----
              ...
              // ----- DÉBUT MODIFICATION POUR DÉTECTION DES FILEURS INDIVIDUELS -----
              // Fonction améliorée de détection des fileurs individuels
             // BLOC AMÉLIORÉ pour détecter et positionner les fileurs individuels
const isFillerType = 
  (obj.piece && obj.piece.type === 'filler') || 
  (obj.piece && obj.piece.description && 
   obj.piece.description.toLowerCase().includes('fileur')) ||
  (obj.id && obj.id.includes('cladding_') && 
   obj.piece && obj.piece.description && 
   obj.piece.description.toLowerCase().includes('fileur'));
...
if (isFillerType) {
  ...
  // Vérifier d'abord le type de position explicite (le plus fiable)
  if (obj.piece && obj.piece.position) {
    isLeftFiller = obj.piece.position === 'left';
    isRightFiller = obj.piece.position === 'right';
    isTopFiller = obj.piece.position === 'top';
  } 
  // Sinon, détecter par l'ID et la description
  else {
    const id = obj.id ? obj.id.toLowerCase() : '';
    const description = obj.piece && obj.piece.description 
      ? obj.piece.description.toLowerCase() 
      : '';
  ...
  console.log(`Fileur individuel détecté: ${obj.piece.description}`, {
    id: obj.id,
    isLeftFiller, 
    isRightFiller, 
    isTopFiller
  ...
  if (isLeftFiller || isRightFiller) {
    ...
    console.log(`Rotation Z (${Math.PI/2} rad) appliquée à un fileur latéral: ${obj.piece.description}`);
  } else if (isTopFiller) {
    // Fileur supérieur - rotation de 90° autour de X
    mesh.rotation.x = Math.PI / 2;
    console.log(`Rotation X (${Math.PI/2} rad) appliquée à un fileur supérieur: ${obj.piece.description}`);
  }
  ...
  mesh.rotation.set(
    ...
    obj.rotation[2] || 0
              ...
              // ----- FIN MODIFICATION POUR DÉTECTION DES FILEURS INDIVIDUELS -----
